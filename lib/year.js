...
Year.setUpDates = (year, dateLine) => {
  ...
  for (let i = dateLine; i < dateLine + 3; i++) {
    ...
    let dayCount = new Date(year, i, 0).getDate();  //returns how many days are in the month
    ...
    eachDateLine[1] = `${allDatesInMonth.substr(21,20)}`;
    eachDateLine[2] = `${allDatesInMonth.substr(42, 20)}`;
    eachDateLine[3] = `${allDatesInMonth.substr(63, 20)}`;
    eachDateLine[4] = `${allDatesInMonth.substr(84, 20)}`;
    eachDateLine[5] = `${allDatesInMonth.substr(105, 20)}`;
    console.log('each date line:  ', eachDateLine[0]);
    console.log('eachDateLine array:  ', eachDateLine);
...
//  };
  const threeMonthDates = `${arrayOfDates[0].join('')}\n`;
...
Year.setUpWholeYear = (year) => {
  return `${genHeading(year)}\n${genMonthNames(year,1)}${genWeekDays()}${genDates(year,1)}${genMonthNames(year,4)}${genWeekDays()}${genMonthNames(year,7)}${genWeekDays()}${genMonthNames(year,10)}${genWeekDays()}`;
};
